...
# This defines the network as it goes from taking
# an observation of the environment to
...
# we apply them.
# We don't just apply gradeients after every episode in order to account
# for noise in the reward signal.
    ...
    while episode_number <= total_episodes:
        ...
        if done:
            ...
            tGrad = sess.run(
                newGrads,
                feed_dict={
                    observations: epx, input_y: epy,
                    advantages: discounted_epr})
            for ix, grad in enumerate(tGrad):
                gradBuffer[ix] += grad

            # If we have completed enough episodes,
            # then update the policy network with our gradients.
            if episode_number % batch_size == 0:
                ...
                # Give a summary of how well our network is doing
