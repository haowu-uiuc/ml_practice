...
if __name__ == '__main__':
    ...
    # Exploration method. Choose between:
        ...
        for i in range(num_episodes):
            ...
            while j < 999:
                ...
                if exploration == "boltzmann":
                    ...
                    # with weights relative to the Q-values.
                    Q_d, allQ = sess.run(
                ...
                if exploration == "bayesian":
                    # Choose an action using a sample from
                    # a dropout approximation of a bayesian q-network.
                    a, allQ = sess.run(
                        [q_net.predict, q_net.Q_out],
                        feed_dict={
                            q_net.inputs: [s],
                ...
                s1, r, d, _ = env.step(a)
                myBuffer.add(np.reshape(np.array(
                    [s, a, r, s1, d]), [1, 5]))
                ...
                if total_steps > pre_train_steps and total_steps % 5 == 0:
                    ...
                    Q1 = sess.run(
                        q_net.predict,
                        feed_dict={
                            q_net.inputs: np.vstack(trainBatch[:, 3]),
                            q_net.keep_per: 1.0})
                    Q2 = sess.run(
                        target_net.Q_out,
                        feed_dict={
                            target_net.inputs: np.vstack(trainBatch[:, 3]),
                    ...
                    _ = sess.run(
                        q_net.updateModel,
                        feed_dict={
                            q_net.inputs: np.vstack(trainBatch[:, 0]),
                            q_net.nextQ: targetQ,
                            q_net.keep_per: 1.0,
            ...
            if i % 100 == 0 and i != 0:
                ...
                if exploration == 'e-greedy':
                    print "Mean Reward: " + str(r_mean) +\
                        " Total Steps: " + str(total_steps) + " e: " + str(e)
                if exploration == 'boltzmann':
                    print "Mean Reward: " + str(r_mean) +\
                        " Total Steps: " + str(total_steps) + " t: " + str(e)
                if exploration == 'bayesian':
                    print "Mean Reward: " + str(r_mean) +\
                        " Total Steps: " + str(total_steps) + " p: " + str(e)
