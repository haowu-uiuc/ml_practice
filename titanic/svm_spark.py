...
test_predictions = model.predict(
    testdata.rdd.map(lambda x: parseFeatures(x[1:]))).map(lambda x: 1 - x)
testIdAndPreds = testdata.rdd.map(lambda p: p[0]).zip(test_predictions)
